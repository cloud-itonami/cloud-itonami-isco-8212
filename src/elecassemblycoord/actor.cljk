(ns elecassemblycoord.actor
  "ElecAssemblyCoordActor — the ISCO-08 8212 electrical and electronic
  equipment assemblers assembly-line scheduling/logistics coordination
  actor as a `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md
  Actors section). One graph run = one line-coordination operation
  ...
  The unconditional invariant: the Electrical/Electronic Assembly Line
  Scheduling Coordination Advisor can never directly commit an
  operating record the ElecAssemblyCoordGovernor refuses, and can
  never make this actor finalize an assembly-execution decision (e.g.
  ...
  an ESD-sensitive workstation safe for handling), and can never
...
(defn build-graph
  "Build a compiled ElecAssemblyCoordActor graph. `store` implements
  `elecassemblycoord.store/Store`. `advisor` implements
  `elecassemblycoord.advisor/Advisor` (defaults to `mock-advisor`).
